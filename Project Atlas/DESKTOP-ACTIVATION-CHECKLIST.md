...
| FollowUps | `FollowUpID`; `CustomerID, SalesActivityID` | `FollowUpID, CustomerID, SalesActivityID, Title, Due At, Owner User ID, Status, Version, Created At, Updated At, Completed At, Cancelled At` | MOS-116 canonical follow-up lifecycle; calendar is optional |
...
| CalendarFollowUpLinks | `CalendarFollowUpLinkID`; `FollowUpID` | `CalendarFollowUpLinkID, FollowUpID, Provider, Calendar ID, External Event ID, External Version, Last Sync Origin, Last Correlation ID, Last Synced FollowUp Version, Created At, Updated At` | Optional calendar adapter link; no calendar identifiers in code |
| ExternalChangeRequests | `ExternalChangeRequestID`; `FollowUpID` | `ExternalChangeRequestID, Provider, FollowUpID, External Event ID, Change Type, Requested Due At, External Version, Status, Details, Detected At, Resolved At, Resolved By, Resolution` | External deletion/conflict review; never auto-deletes MOS follow-up |
