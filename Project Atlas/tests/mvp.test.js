...
context.createRepository_ = (entity) => ({
  ...
  findById: (id) => { const item = records[entity].find((r) => r.id === id); if (!item) throw new context.VmosNotFoundError_('missing'); return item; },
  insert: (item) => { records[entity].push({ ...item }); return item; },
  updateById: (id, changes) => { if (Object.prototype.hasOwnProperty.call(changes, 'id')) throw new context.VmosValidationError_('Primary key cannot be changed.'); const item = records[entity].find((r) => r.id === id); if (!item) throw new context.VmosNotFoundError_('missing'); Object.keys(changes).forEach((key) => { item[key] = changes[key]; }); return item; }
});
...
const mappedRecord = context.SheetsRepository_.prototype.toDomain_.call({}, [sheetDate, 'RFQ-26-0128', 4, false, ''], { createdAt: { column: 1 }, id: { column: 2 }, quantity: { column: 3 }, active: { column: 4 }, blank: { column: 5 } });
...
const customer = new context.MvpService_('Customer').create({ name: 'Acme' });
assert.equal(customer.id, 'CUST-26-0001');
assert.equal(customer.createdBy, 'operator@example.com');
const authoritativeCustomer = new context.MvpService_('Customer', { auditUser: () => 'USR-AUTHORITATIVE' }).create({ name: 'Secure Co', createdBy: 'FORGED', updatedBy: 'FORGED' });
assert.equal(authoritativeCustomer.createdBy, 'USR-AUTHORITATIVE');
const authoritativeUpdated = new context.MvpService_('Customer', { auditUser: () => 'USR-UPDATER' }).update(authoritativeCustomer.id, { name: 'Secure Co 2', createdBy: 'FORGED', updatedBy: 'FORGED' });
assert.equal(authoritativeUpdated.createdBy, 'USR-AUTHORITATIVE');
assert.equal(authoritativeUpdated.updatedBy, 'USR-UPDATER');
const rfq = new context.MvpService_('RFQ').create({ customerId: customer.id, description: 'Bracket' });
const updatedRfq = new context.MvpService_('RFQ').update(rfq.id, { description: 'Revised bracket' });
...
assert.throws(() => new context.MvpService_('RFQ').update(rfq.id, { id: 'RFQ-26-9999' }), /Primary key/);
assert.throws(() => new context.MvpService_('RFQ').update(rfq.id, { description: '' }), /required/);
const quote = new context.MvpService_('Quote').create({ rfqId: rfq.id });
const job = new context.MvpService_('Job').create({ quoteId: quote.id });
const invoice = new context.MvpService_('Invoice').create({ jobId: job.id, total: 1250 });
...
assert.throws(() => new context.MvpService_('RFQ').create({ customerId: customer.id }), /required/);
