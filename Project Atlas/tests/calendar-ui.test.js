...
['Atlas account','Connected calendars','Follow-Up due:','Not scheduled','SCHEDULE','Calendar event deleted','KEEP FOLLOW-UP','MARK COMPLETE','CANCEL FOLLOW-UP','USE ATLAS TIME','USE CALENDAR TIME','CHOOSE ANOTHER TIME','Operational schedule','Read-only subscription','@media(max-width:900px)','@media(max-width:600px)'].forEach(text=>assert.ok(ui.includes(text),'calendar UI includes '+text));
...
['getCalendarWorkspace','disconnectCalendarConnection','resolveCalendarExternalChange','retryCalendarCleanup','acknowledgeCalendarCleanup','reassignFollowUp','resolveAtlasRoute_(e)','createCalendarFollowUpOrchestration_(a.checkpoint).schedule','createCalendarFollowUpOrchestration_(a.checkpoint).reassign'].forEach(text=>assert.ok(code.includes(text),'calendar endpoint/route includes '+text));
['CLEANUP_FAILED','RETRY CLEANUP','ACKNOWLEDGE','Your Atlas change was saved, but the previous calendar event could not be removed.'].forEach(text=>assert.ok(ui.includes(text),'cleanup review UI includes '+text));
const cleanupBranch=ui.slice(ui.indexOf("if(r.changeType==='CLEANUP_FAILED')"),ui.indexOf("var external=",ui.indexOf("if(r.changeType==='CLEANUP_FAILED')")));
['KEEP FOLLOW-UP','MARK COMPLETE','CANCEL FOLLOW-UP'].forEach(text=>assert.ok(!cleanupBranch.includes(text),'cleanup review excludes deletion action '+text));
