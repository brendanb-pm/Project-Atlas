...
['getCalendarWorkspace','disconnectCalendarConnection','resolveCalendarExternalChange','retryCalendarCleanup','acknowledgeCalendarCleanup','reassignFollowUp',"parameter && e.parameter.calendar === '1'",'createCalendarFollowUpOrchestration_(a.checkpoint).schedule','createCalendarFollowUpOrchestration_(a.checkpoint).reassign'].forEach(text=>assert.ok(code.includes(text),'calendar endpoint/route includes '+text));
['CLEANUP_FAILED','RETRY CLEANUP','ACKNOWLEDGE','Your MOS change was saved, but the previous calendar event could not be removed.'].forEach(text=>assert.ok(ui.includes(text),'cleanup review UI includes '+text));
