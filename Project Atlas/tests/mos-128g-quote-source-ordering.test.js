...
const errorStart=html.indexOf('function showError('),errorEnd=html.indexOf('function picker(');
vm.runInContext(html.slice(errorStart,errorEnd),context);
...
ui=reset('targets','sourceTargetSearch','sourceTargetResults');ui.input.value='FAIL';context.sourceTargetSearch();requests.targets[0].failure(new Error('active target failure'));assert.equal(ui.input.getAttribute('aria-busy'),undefined);ui.input.value='RETRY';context.sourceTargetSearch();requests.targets[1].success(ok('RETRY'));assert.match(ui.result.html,/>RETRY</);assert.equal(ui.input.getAttribute('aria-busy'),undefined);

// Generic workspace/refresh errors cannot claim busy-state ownership from either active picker.
ui=reset('documents','sourceSearch','sourceResults');ui.input.value='ACTIVE-DOCUMENT';context.sourceSearch();context.showError(new Error('unrelated workspace failure'));assert.equal(ui.input.getAttribute('aria-busy'),'true');requests.documents[0].success(ok('ACTIVE-DOCUMENT'));assert.equal(ui.input.getAttribute('aria-busy'),undefined);
ui=reset('targets','sourceTargetSearch','sourceTargetResults');ui.input.value='ACTIVE-TARGET';context.sourceTargetSearch();context.showError(new Error('unrelated refresh failure'));assert.equal(ui.input.getAttribute('aria-busy'),'true');requests.targets[0].success(ok('ACTIVE-TARGET'));assert.equal(ui.input.getAttribute('aria-busy'),undefined);

// Independent domains: starting a newer source request does not invalidate a valid target response.
reset('documents','sourceSearch','sourceResults');reset('targets','sourceTargetSearch','sourceTargetResults');element('sourceSearch').value='DOC-A';context.sourceSearch();element('sourceTargetSearch').value='TARGET-X';context.sourceTargetSearch();element('sourceSearch').value='DOC-B';context.sourceSearch();requests.targets[0].failure(new Error('target failure'));assert.equal(element('sourceSearch').getAttribute('aria-busy'),'true');requests.documents[0].failure(new Error('stale source failure'));assert.equal(element('sourceSearch').getAttribute('aria-busy'),'true');requests.documents[1].failure(new Error('active source failure'));assert.equal(element('sourceSearch').getAttribute('aria-busy'),undefined);element('sourceSearch').value='DOC-RETRY';context.sourceSearch();requests.documents[2].success(ok('DOC-RETRY'));assert.match(element('sourceResults').html,/>DOC-RETRY</);assert.equal(element('sourceSearch').getAttribute('aria-busy'),undefined);
