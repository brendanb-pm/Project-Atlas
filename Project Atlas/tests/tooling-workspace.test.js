...
assert.match(html, /ACTUAL measured diameter[\s\S]*recordToolRegrind/, 'manual post-regrind measurement is explicit and authoritative');
assert.match(html, /NOMINAL geometry stays[\s\S]*ACTUAL geometry/, 'operator copy prevents nominal/actual substitution');
assert.match(html, /getToolingHistory\(\{toolInstanceId:toolId,limit:25\}\)/, 'tool history is operator-visible and bounded');
assert.match(html, /evidenceAttachmentId:source\?source\.value:null/, 'manual regrind can preserve selected MOS-140A evidence');
assert.match(html, /replaceToolPhysicalIdentifier/, 'physical labels can be assigned or replaced');
assert.match(html, /does not grant access/, 'physical identifiers are described as locators rather than authority');
assert.match(html, /if\(version!==historyRequestVersion\)return/, 'late history responses cannot overwrite current tool state');
assert.match(html, /prefers-reduced-motion:reduce/, 'loading animation respects reduced-motion preference');
