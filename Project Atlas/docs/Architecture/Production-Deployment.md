...
Basic Follow-Up persistence can be activated only after reviewing the exact schemas in `MOS-122-ACT2-Live-Workbook-Activation.md`. An authoritative Atlas administrator may then run `initializeFollowUpPersistence()`. It preflights both `FollowUps` and `FollowUpEvents`, creates only absent sheets with exact headers, and stops before creating either missing sheet if an existing target is incompatible. It does not activate or create calendar-provider persistence.

After the exact purchasing worksheet, mapping, threshold, and approval policy are approved, an authoritative Atlas administrator may run `initializePurchaseApprovalPersistence()`. It reads the validated Script Property mapping and threshold, creates only the absent configured worksheet with exact mapped headers, and refuses incompatible existing headers. It never chooses or changes the mapping or threshold and seeds no business records.
