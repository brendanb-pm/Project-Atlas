...
### Validation-deployment entry contract

In the current Google Workspace validation deployment, the base URL and every protected deep link are resolved through a strict server-side entry check before Atlas selects a workspace template. A non-empty `Session.getActiveUser()` value is only a principal candidate: it must map through an active `ExternalIdentityReference` to an active `AtlasUser`, the configured tenant's active membership, an allowed commercial entitlement, and at least one applicable capability. Browser callables use the same fail-closed rule even while `ATLAS_IDENTITY_ENFORCEMENT_MODE=VALIDATION`; they never receive `VALIDATION_UNENFORCED` authority. Missing principal renders Sign In, while a mapped user whose membership, entitlement, or capabilities are unavailable receives a generic access-unavailable surface without tenant or security details. Protected deep links preserve only an allow-listed route token and discard record identifiers before authorization.

This validation composition does not turn the Apps Script principal into a provider-neutral Atlas application session. Browser `sessionStorage` tokens still cannot be presented securely to `doGet` or every `google.script.run` request under the current host, so Atlas-session expiry/revocation and authoritative logout for Microsoft/Google OIDC remain dependent on the external session edge described above. The validation path proves the configured Workspace identity boundary only; it must not be represented as live OIDC activation.
