...
function getPurchaseApprovalConfig_() {
  ...
  if (!rawMapping) throw new VmosConfigurationError('VMOS_PURCHASE_APPROVAL_MAPPING is not configured. Purchase approvals are disabled; no sheet was changed.');
  var mapping;
  try { mapping = JSON.parse(rawMapping); } catch (error) { throw new VmosConfigurationError('VMOS_PURCHASE_APPROVAL_MAPPING must be valid JSON.'); }
  ...
  if (!mapping || !mapping.sheetName || !mapping.fields || !mapping.idField) throw new VmosConfigurationError('Purchase approval mapping requires sheetName, idField, and fields.');
  requiredFields.forEach(function (field) {
    if (!Array.isArray(mapping.fields[field]) || !mapping.fields[field].length) throw new VmosConfigurationError('Purchase approval mapping is missing field "' + field + '".');
  });
  ...
  if (rawThreshold === null || rawThreshold === '' || isNaN(Number(rawThreshold)) || Number(rawThreshold) < 0) {
    throw new VmosConfigurationError('VMOS_PURCHASE_APPROVAL_THRESHOLD must be a non-negative number.');
  }
