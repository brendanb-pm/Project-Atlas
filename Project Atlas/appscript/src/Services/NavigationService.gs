...
function atlasRouteTitle_(route){var normalized=normalizeAtlasRoute_(route),authTitle={'sign-in':'Sign in','auth-callback':'Completing sign in'}[normalized],found=ATLAS_ROUTE_REGISTRY.filter(function(item){return item.id===normalized;})[0];return (authTitle||(found?found.label:'Command Center'))+' - Atlas';}
