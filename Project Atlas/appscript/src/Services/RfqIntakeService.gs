...
function RfqIntakeService_(dependencies) {
...
RfqIntakeService_.prototype.poll = function () {
  if (!this.enabled()) return { enabled: false, processed: 0, proposals: [] };
  if (!this.gmail || !this.ai || !this.staging) throw new VmosConfigurationError_('RFQ Intake providers and staging storage must be configured before polling.');
...
RfqIntakeService_.prototype.listPending = function () { return this.staging.listByStatus('PENDING_REVIEW'); };
RfqIntakeService_.prototype.reject = function (id, note, reviewer) { var record = this.staging.get(id); if (record.status === 'REJECTED') return record; record.status = 'REJECTED'; record.rejectionReason = note || ''; record.reviewedBy = reviewer || ''; record.reviewedAt = new Date(); record.audit.push({ type: 'REJECTED', note: note || '', at: new Date() }); return this.staging.save(record); };
RfqIntakeService_.prototype.approve = function (id, correctedProposal) {
  if (!this.enabled()) throw new VmosConfigurationError_('RFQ Intake is disabled. No production record was created.');
  var record = this.staging.get(id); if (record.status === 'APPROVED_FOR_PLAN') return record.result;
  if (record.status !== 'PENDING_REVIEW') throw new VmosValidationError_('Only a pending intake proposal can be approved.');
...
function validateRfqIntakeProposal_(proposal) {
  proposal = proposal || {};
  if (!proposal.description || typeof proposal.description !== 'string') throw new VmosValidationError_('AI proposal requires a text description.');
  if (proposal.customerId !== undefined && typeof proposal.customerId !== 'string') throw new VmosValidationError_('Customer ID must be a string when supplied.');
  if (proposal.dueDate !== undefined && isNaN(new Date(proposal.dueDate).getTime())) throw new VmosValidationError_('Due date must be a valid date when supplied.');
  if (proposal.parts !== undefined && !Array.isArray(proposal.parts)) throw new VmosValidationError_('Parts must be an array.');
  var parts = (proposal.parts || []).map(function (part) { if (!part.partNumber) throw new VmosValidationError_('Every part requires an exact part number.'); return { partNumber: String(part.partNumber), revision: part.revision || '', description: part.description || '', material: part.material || '', processes: part.processes || [], supplier: part.supplier === 'VITALITY' ? 'VITALITY' : 'CUSTOMER', customerSuppliedComponents: part.customerSuppliedComponents || [], quantities: part.quantities || [], confidence: part.confidence || {} }; });
...
function VmosProposalProvider_() {}
