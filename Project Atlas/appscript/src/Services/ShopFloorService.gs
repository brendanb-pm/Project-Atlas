...
function ShopFloorService(dependencies) {
...
ShopFloorService.prototype.configureJob = function (jobId, workflowId, initialStatus) {
  ...
  if (!isValidOpaqueJobQrToken_(token)) throw new VmosConfigurationError('Secure QR token generation failed. No token was created.');
...
ShopFloorService.prototype.activeQr_ = function (token) {
  if (!isValidOpaqueJobQrToken_(token)) throw new VmosNotFoundError('This QR code is invalid or no longer available.');
  try {
    var record = this.qrTokens.findByToken(String(token));
    if (!record || record.revokedAt) throw new VmosNotFoundError('inactive');
    return record;
  } catch (error) {
    if (error && error.code === 'NOT_FOUND') throw new VmosNotFoundError('This QR code is invalid or no longer available.');
    throw error;
  }
};

ShopFloorService.prototype.assertQrScope_ = function (token, jobId) {
  var record = this.activeQr_(token);
  if (String(record.jobId) !== String(jobId)) throw new VmosNotFoundError('This QR code is invalid or no longer available.');
  return record;
};

/** Lifecycle operations remain server-side until MOS-121 identity enforcement gates an admin endpoint. */
ShopFloorService.prototype.revokeQr = function (token, reason) {
...
ShopFloorService.prototype.rotateQr = function (jobId, reason) {
  var self = this, lock = LockService.getScriptLock(); lock.waitLock(30000);
  try {
    var active = this.qrTokens.findActiveByJobId(jobId)[0];
    if (!active) throw new VmosNotFoundError('No active QR code is available to replace.');
    var job = this.jobs.get(jobId), actor = this.auditUser(), token = this.tokenGenerator();
    if (!isValidOpaqueJobQrToken_(token)) throw new VmosConfigurationError('Secure QR token generation failed. The existing token remains active.');
...
ShopFloorService.prototype.transition = function (jobId, targetStatus, commandId, notes, token) {
  var self = this, scopedQr = this.assertQrScope_(token, jobId);
  return this.withCommand_(jobId, commandId, scopedQr, function (job, qr) {
...
ShopFloorService.prototype.reportProblem = function (jobId, payload, commandId, token) {
  ...
  var scopedQr = this.assertQrScope_(token, jobId);
  return this.withCommand_(jobId, commandId, scopedQr, function (job, qr) {
    if (String(job.status || '').toUpperCase() === 'COMPLETE') throw new VmosValidationError('Completed work cannot be changed from the shop-floor QR workflow.');
...
ShopFloorService.prototype.resolveBlock = function (jobId, payload, commandId, token) {
...
ShopFloorService.prototype.withCommand_ = function (jobId, commandId, qr, action) {
