...
SecurityAuditService_.prototype.executionContext_=function(context,record,options){var values={};Object.keys(context).forEach(function(key){values[key]=context[key];});values.checkpoint=function(result){return this.checkpoint(record,result,options);}.bind(this);return Object.freeze(values);};
SecurityAuditService_.prototype.checkpoint=function(record,value,options){var result=this.safeResult_(value||{},options||{}),resourceId=String(options&&options.resourceId||result.id||record.resourceId||'');return this.repository.update(record.id,{resourceId:resourceId,mutationState:'CANONICAL_COMPLETED',mutationAt:this.clock(),resultJson:JSON.stringify(result),lastAttemptAt:this.clock()});};
...
SecurityAuditService_.prototype.markRecovered=function(record,result,recoveryContext){recoveryContext=recoveryContext||{};return this.withLock_(function(){var current=this.repository.get?this.repository.get(record.id):record;if(String(current.status).toUpperCase()==='COMPLETED')return current;return this.repository.update(record.id,{resultCode:'SUCCEEDED',resultJson:JSON.stringify(this.safeResult_(result||{},{})),recoveryStatus:'COMPLETED',recoveryActor:String(recoveryContext.actor||'SYSTEM:SECURITY_OPERATION_RECOVERY'),recoveryCorrelationId:String(recoveryContext.correlationId||''),reconciledAt:this.clock(),completedAt:this.clock(),outcome:'SUCCEEDED',status:'COMPLETED',lastAttemptAt:this.clock(),leaseExpiresAt:''});}.bind(this));};
