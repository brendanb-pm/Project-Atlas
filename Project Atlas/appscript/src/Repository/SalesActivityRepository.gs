...
SalesActivityRepository_.prototype.listByCustomerId=function(customerId){return this.list().filter(function(row){return String(row.customerId)===String(customerId);});};
SalesActivityRepository_.prototype.listOpen=function(){return this.list().filter(function(row){return ['OPEN','FOLLOW_UP_DUE','OVERDUE'].indexOf(row.status)!==-1;});};
